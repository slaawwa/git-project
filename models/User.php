...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public static function next7pro($maxNpp) {
      // echo $maxNpp.'<hr>';
      $all_do7pro = static::findByRole('do-7progress');
      $allArr = [];
      foreach ($all_do7pro as $value) {
        $allArr[] = $value['id'];
      }
      if (!count($allArr)) {
        return false;
      }
      $last_user = Question::find()->where(['npp'=>$maxNpp])->one()->user_take;
      // echo '<pre>last_user: '.print_r($last_user, true).'</pre>';
      // echo '<pre>'.print_r($allArr, true).'</pre>';
      // echo '<hr>';
      $current_pos = array_search($last_user, $allArr);
      if ($current_pos === false) {
        // $current_pos = '-';
        $Next = 0;
      } else {
        $Next = isset($all_do7pro[$current_pos+1])? $current_pos+1: 0;
        // $current_pos = '+';
      }
      /*echo '<pre>next_pos: '.print_r($Next, true).'</pre>';
      echo '<pre>next_user: '.print_r($allArr[$Next], true).'</pre>';
      die('-'.$current_pos.'-');*/
      return $allArr[$Next];
    }
    ...
    public static function checkQ() {
        ...
        foreach ($res as $a) {
            ...
            $a->user_take = User::next7pro($maxNpp); // Need::day2user($wday3);
